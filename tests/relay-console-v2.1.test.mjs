...
const htmlPath=fileURLToPath(new URL("../relay-console-v2.1.0.html",import.meta.url));
...
globalThis.__relayTest={
  ...
  sessionHasMeaningfulWork,RECIPES,MAX_PARTICIPANTS,Store,setRecipe,transcriptMd,I18N,tr,setUiLocale,setPromptLocale,loadedRoleSet,
  setPromptReply(value){globalThis.__promptReply=value;},setConfirmReply(value){globalThis.__confirmReply=value;},
  setState(value){state=value;},getState(){return state;},getRecipe(){return recipe;},getUiLocale(){return uiLocale;},getPromptLocale(){return promptLocale;}
};`;
vm.runInContext(html.slice(scriptStart,scriptEnd)+exportsCode,sandbox,{filename:"relay-console-v2.1.0.html"});
...
function stateFor(turns,participants,answers){
  return {
    version:"2.1.0",question:"Which answer is strongest?",recipe:"ballot",mode:"blind",rounds:1,closing:true,format:"markdown",uiLocale:"en",promptLocale:"en",nonce:"RXTEST1234",
...
test("embedded JavaScript loads in a minimal browser environment",()=>{
  ...
  assert.match(html,/<title>Relay Console v2\.1\.0<\/title>/);
  assert.match(html,/const VERSION="2\.1\.0";/);
  assert.doesNotMatch(html,/2\.1\.0-draft/);
...
test("active v2.1 product and release text contains no em dashes",()=>{
  const paths=[
    htmlPath,
    ...
    fileURLToPath(new URL("../docs/v2.1.0-release-notes.md",import.meta.url)),
    fileURLToPath(new URL("../docs/v2.1.0-release-audit.md",import.meta.url))
  ];
...
test("English and French catalogs have identical keys and placeholders",()=>{
  const enKeys=Object.keys(app.I18N.en).sort();
  const frKeys=Object.keys(app.I18N.fr).sort();
  assert.deepEqual(frKeys,enKeys);
  const placeholders=value=>Array.from(String(value).matchAll(/\{([A-Za-z0-9_]+)\}/g),m=>m[1]).sort();
  for(const key of enKeys) assert.deepEqual(placeholders(app.I18N.fr[key]),placeholders(app.I18N.en[key]),key);
});

test("every declarative UI translation key exists in both catalogs",()=>{
  const keys=Array.from(html.matchAll(/data-i18n(?:-html|-title|-placeholder|-aria)?="([^"]+)"/g),m=>m[1]);
  assert.ok(keys.length>40);
  for(const key of keys){
    assert.ok(key in app.I18N.en,`English: ${key}`);
    assert.ok(key in app.I18N.fr,`French: ${key}`);
  }
});
...
test("interface and prompt language preferences remain independent with English fallback",()=>{
  ...
  assert.equal(app.tr("es","question.heading"),"The question");
